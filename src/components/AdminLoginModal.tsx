...
export const AdminLoginModal: React.FC = () => {
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError(language === 'bn' ? 'দয়া করে আপনার গোপন পাসওয়ার্ড লিখুন।' : 'Please enter your secret password.');
      return;
    }

    const ok = loginAdmin(trimmed);
    if (ok) {
      setSuccess(true);
      setError(null);
      setTimeout(() => {
      ...
      setError(language === 'bn' ? 'ভুল পাসওয়ার্ড! দয়া করে সঠিক পাসওয়ার্ড দিন।' : 'Incorrect password! Access denied.');
    }
  };
    ...
    >
      ...
      >
        ...
        <h3 className="font-display font-extrabold text-2xl tracking-tight text-white mb-1.5">
          {language === 'bn' ? 'অ্যাডমিন সিকিউরিটি পোর্টাল' : 'Admin Security Access'}
        </h3>
        <p className="text-xs sm:text-sm text-slate-400 mb-6 leading-relaxed">
          {language === 'bn'
            ? 'অ্যাডমিন কন্ট্রোল সেন্টারে প্রবেশ করতে আপনার ব্যক্তিগত গোপন পাসওয়ার্ড দিন।'
            : 'Enter your authorized security password to access the admin panel.'}
        </p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
              {language === 'bn' ? 'গোপন পাসওয়ার্ড' : 'Secret Password'}
            </label>
            ...
            <div className="relative">
              ...
              <input
                ...
                autoFocus
                autoComplete="off"
                className="w-full pl-10 pr-12 py-3 rounded-xl border border-slate-700 bg-slate-950 text-white placeholder:text-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all font-mono"
              />
              <button
                type="button"
          ...
          >
            <span>{language === 'bn' ? 'লগইন করুন' : 'Secure Login'}</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </form>

        {/* Security Warning Notice - No password hint! */}
        <div className="mt-5 pt-4 border-t border-slate-800 text-[11px] text-slate-400 flex items-start gap-2.5">
          <ShieldAlert className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
          <p className="leading-relaxed">
            {language === 'bn'
              ? 'নিরাপত্তা বিজ্ঞপ্তি: এটি সোহানের ব্যক্তিগত অ্যাডমিন পোর্টাল। শুধুমাত্র অনুমোদিত গোপন পাসওয়ার্ড দিয়ে প্রবেশ করা যাবে।'
              : 'Security Notice: This is Sohan\'s private admin portal. Access is strictly restricted to authorized owner.'}
          </p>
        </div>
      </div>
    </div>
